    ...
    for a in range(hull_cube.data.shape[1]):
        for b in range(hull_cube.data.shape[2]):
            ...
            if input_hull[39] == 0:
                ...
                final_index = min(max(min2000p[0] + minimum_2000 - ofset, midpointp + 1), 73)
                ...
                if input_hull[min2000p+minimum_2000-ofset][0] >= 0.983:
                    ...
                    stack_min2000.append(final_2000)
...
def continuum_removal_SAFO (filtered_cube,wavelengths,order1,order2):
    ...
    for a in range(filtered_cube.data.shape[1]):
        for b in range(filtered_cube.data.shape[2]):
            ...
            SAFO_cube=filtered_cube.data[:,a,b]  
            
            if SAFO_cube[39] == 0: 
                stack_SAFO.append(np.zeros(83))
            else:
                ...
                polival2000=np.polyval(fit2000,wavelengths[42:83])
